defmodule Timesheet.SheetsTest do
  ...
  describe "sheets" do
    ...
    @valid_attrs %{approve: true, date: ~D[2010-04-17], worker_id: 42}
    @update_attrs %{approve: false, date: ~D[2011-05-18], worker_id: 43}
    @invalid_attrs %{approve: nil, date: nil, worker_id: nil}
    ...
    test "create_sheet/1 with valid data creates a sheet" do
      ...
      assert sheet.approve == true
      assert sheet.date == ~D[2010-04-17]
      assert sheet.worker_id == 42
    end
    ...
    test "update_sheet/2 with valid data updates the sheet" do
      ...
      assert sheet.approve == false
      assert sheet.date == ~D[2011-05-18]
      assert sheet.worker_id == 43
    end
